...
public class AnimalDAO implements IAnimalDAO {
    ...
    public void insert(Animal customer) { // Реализация вставки новой записи
        ...
        insert.update("INSERT INTO edktable__6 (NAME_ANIMAL, COLOR_ANIMAL) VALUES(?,?)",
    ...
    public void append(String nameAnimal, String colorAnimal) {  // Реализация добавления новой записи
        JdbcTemplate insert = new JdbcTemplate(dataSource);
        insert.update("INSERT INTO edktable__6 (NAME_ANIMAL, COLOR_ANIMAL) VALUES(?,?)", new Object[]{nameAnimal, colorAnimal});
    }

    @Override
    public void deleteByColorAnimal(String colorAnimal) {  // Реализация удаления записей по цвету
        JdbcTemplate insert = new JdbcTemplate(dataSource);
        insert.update("DELETE FROM edktable__6 WHERE COLOR_ANIMAL LIKE ?", new Object[]{'%' + colorAnimal + '%'});
    }
        ...
        transactionTemplate.execute(new TransactionCallback() {
            ...
            public Object doInTransaction(TransactionStatus status) {
                ...
                try {
                    ...
                    delete.update("DELETE from edktable__6 where NAME_ANIMAL = ? AND COLOR_ANIMAL = ?", new Object[]{nameAnimal, colorAnimal});
    ...
    public void deleteAll() {  // Реализация удаления всех запией
        JdbcTemplate delete = new JdbcTemplate(dataSource);
        delete.update("DELETE from edktable__6");
    }

    @Override
    public void update(String oldColorAnimal, String newColorAnimal) {  // Изменение записей в таблице
        JdbcTemplate update = new JdbcTemplate(dataSource);
        update.update("UPDATE edktable__6 SET COLOR_ANIMAL = ? WHERE COLOR_ANIMAL = ?", new Object[]{newColorAnimal, oldColorAnimal});
    }

    @Override
    public List<Animal> findByNameAnimal(String nameAnimal) {  // Реализация поиска записей по названию
        JdbcTemplate select = new JdbcTemplate(dataSource);
        String sql = "SELECT * FROM edktable__6 WHERE NAME_ANIMAL LIKE ?";
    ...
    public List<Animal> select(String nameAnimal, String colorAnimal) {  // Реализация получения записей с заданными названием и цветом
        JdbcTemplate select = new JdbcTemplate(dataSource);
        return select.query("select  * from edktable__6 where NAME_ANIMAL = ? AND COLOR_ANIMAL= ?",
    ...
    public List<Animal> selectAll() {  // Реализация получения всех записей
        JdbcTemplate select = new JdbcTemplate(dataSource);
        return select.query("select * from edktable__6", new AnimalRowMapper());
    }
    ...
    public void delete(final String nameAnimal) {  // Реализация удаления записей с указанными названием и цветом
        ...
        transactionTemplate.execute(new TransactionCallback() {
            ...
            public Object doInTransaction(TransactionStatus status) {
                ...
                try {
                    JdbcTemplate delete = new JdbcTemplate(dataSource);
                    delete.update("DELETE from edktable__6 where NAME_ANIMAL = ?", new Object[]{nameAnimal});
                } catch (RuntimeException e) {
                    status.setRollbackOnly();
                    throw e;
                } catch (Exception e) {
